<p align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="120" />
</p>

<h1 align="center">Hi, I'm <strong>Rootinj</strong> 👋</h1>

<h3 align="center">
BCA Graduate • Web Development & UI/UX • Learning Gen AI & Databases
</h3>

<p align="center">
  <em>"Designing solutions with clean code and clean UI."</em>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Role-BCA%20Graduate-1976d2?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Focus-Gen%20AI%20%26%20Databases-388e3c?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Loves-Web%20Dev%20%26%20UI%2FUX-f57c00?style=for-the-badge" />
</p>

---

## 🧑‍💻 About Me

- 🎓 **BCA Graduate** with a strong interest in full-stack development.
- 🤖 Currently **learning Generative AI** (LLMs, prompts, AI apps) and **Databases**.
- 💻 Comfortable with **web development** and passionate about **UI/UX design**.
- 🌱 Always experimenting with **frontend clones, portfolios, and clean layouts**.
- 🤝 Open to collaborate on:
  - Web apps  
  - UI clones  
  - Beginner-friendly Gen AI projects  

---

## 🎯 Current Learning Roadmap

- 📌 **Gen AI**: prompts, basic LLM concepts, integrating AI into apps  
- 🗄️ **Databases**: SQL & NoSQL, queries, schemas, basic optimization  
- 🎨 **UI/UX**: modern layouts, responsive design, typography, color systems  
- 🌐 **Web Dev**: improving frontend structure and clean reusable components  

---

## 🛠️ Tech Stack

### 💻 Languages
<p>
  <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=oracle&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
</p>

### 🎨 Frontend & UI
<p>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/SCSS-CC6699?style=for-the-badge&logo=sass&logoColor=white" />
  <img src="https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwindcss&logoColor=white" />
</p>

### 🗄️ Backend & Databases (Learning)
<p>
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white" />
  <img src="https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white" />
</p>

### ⚙️ Tools & Workflow
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white" />
</p>

---

## 📂 Featured Projects

- 🚀 **[PW-skills-Clone-Using-Tailwind-CSS](https://github.com/rootinj/PW-skills-Clone-Using-Tailwind-CSS)**  
  A homepage clone built using **Tailwind CSS**, focusing on responsive layout and modern UI.

- 🎨 **[my-Portfolio](https://github.com/rootinj/my-Portfolio)**  
  A personal **portfolio website** experimenting with **CSS, SCSS, JS & clean UI design**.

- 🧾 **[all-done](https://github.com/rootinj/all-done)**  
  A frontend project practicing layout, sections, and simple component structure.

- 🌐 **[testCapstone](https://github.com/rootinj/testCapstone)**  
  A basic project setup used for practicing and testing frontend concepts.

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=rootinj&show_icons=true&theme=material-palenight" alt="GitHub Stats" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rootinj&layout=compact&theme=material-palenight" alt="Top Languages" />
</p>

---

## 🤝 Connect With Me

<p>
  <a href="https://linkedin.com/in/your-linkedin" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://instagram.com/your-insta" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>
  <a href="mailto:your-email@example.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

<p align="center">
  Thanks for visiting my profile! ⭐  
  <br />
  <sub>Actively learning • Actively building • Actively improving</sub>
</p>
